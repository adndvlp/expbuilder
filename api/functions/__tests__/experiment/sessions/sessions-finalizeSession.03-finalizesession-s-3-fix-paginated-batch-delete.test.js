...
describe("finalizeSession — S-3 fix: paginated batch delete", () => {
  test("for >500 trials, calls db.batch() multiple times (no single >500 commit)", async () => {
    ...
    // 1200 trials → should result in 3 batches of (500, 500, 200)
    const trials = Array.from({ length: 1200 }, (_, i) => ({
      id: `t${i}`,
      data: { clientTimestamp: i, trial_index: i, v: i },
    }));
    const snap = makeSnapshot(trials);
    ...
    // Should call db.batch() exactly 3 times (ceil(1200/500))
    expect(fs.db.batch).toHaveBeenCalledTimes(3);
    // Session doc deleted via direct .delete (not in batch)
