...
describe("apiData — no-action routing by body shape", () => {
  test("experimentID + sessionId + !data → handleCreateSession", async () => {
    const res = makeRes();
    const req = makeReq({ body: { experimentID: "EID", sessionId: "S1" } });
    await apiData(req, res);
    expect(mockHandleCreateSession).toHaveBeenCalledWith(req, res, "EID", "S1");
    expect(mockHandleAppendResult).not.toHaveBeenCalled();
  });

  test("experimentID + sessionId + data → handleAppendResult", async () => {
    const res = makeRes();
    const req = makeReq({
      body: { experimentID: "EID", sessionId: "S1", data: { trial: 1 } },
    });
    await apiData(req, res);
    expect(mockHandleAppendResult).toHaveBeenCalledWith(
      req,
      res,
      "EID",
      "S1",
