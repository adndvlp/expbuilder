...
describe("handleDownloadSession — error branches", () => {
  ...
  test("falls back to result.error when storage returns no errorText", async () => {
    fs.getRef("experiments/EID").get.mockResolvedValueOnce({
      exists: true,
      data: () => ({
    ...
    mockDownloadSession.mockResolvedValueOnce({
      success: false,
    ...
    await handleDownloadSession(makeReq({}), res, "EID", "S1");
    expect(res.statusCode).toBe(400);
    expect(res.jsonBody.message).toBe("FILE_NOT_FOUND");
  });

  test("default message when storage returns success:false without errorText or error", async () => {
    fs.getRef("experiments/EID").get.mockResolvedValueOnce({
      exists: true,
      data: () => ({
        storageProvider: "dropbox",
    ...
    mockDownloadSession.mockResolvedValueOnce({ success: false });
    const res = makeRes();
    await handleDownloadSession(makeReq({}), res, "EID", "S1");
    expect(res.statusCode).toBe(400);
    expect(res.jsonBody.message).toBe("Error downloading session");
  });
