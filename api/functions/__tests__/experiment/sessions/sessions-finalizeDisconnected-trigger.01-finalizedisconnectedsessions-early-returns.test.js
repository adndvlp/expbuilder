...
describe("finalizeDisconnectedSessions — early returns", () => {
  test("returns null when afterData is null (node deleted)", async () => {
    const ev = makeEvent({ before: { connected: true }, after: null });
    const r = await finalizeDisconnectedSessions(ev);
    expect(r).toBeNull();
    expect(ev.data.after.ref.update).not.toHaveBeenCalled();
  });

  test("returns null when finalizationProcessed is already true", async () => {
    const ev = makeEvent({
      before: { connected: true },
      after: { connected: false, finalizationProcessed: true },
    });
    const r = await finalizeDisconnectedSessions(ev);
    expect(r).toBeNull();
    expect(ev.data.after.ref.update).not.toHaveBeenCalled();
