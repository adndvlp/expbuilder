...
describe("deleteExperiment — RTDB failure isolation", () => {
  test("if RTDB cleanup fails, returns success with rtdbWarning + rtdbCleared=false", async () => {
    ...
    rtdbRefRemove.mockRejectedValueOnce(new Error("rtdb auth fail"));

    const r = await deleteExperiment("EID", "u1");
    expect(r.success).toBe(true);
    expect(r.rtdbCleared).toBe(false);
    expect(r.rtdbWarning).toBe("rtdb auth fail");
  });
});
