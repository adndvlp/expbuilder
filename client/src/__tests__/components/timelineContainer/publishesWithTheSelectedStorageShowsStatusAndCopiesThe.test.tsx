...
import {
  confirmGitHubPagesPublish,
  copyPublishedPagesLink,
  enableAuthenticatedPublishing,
} from "./publishingHarness";
...
describe("Timeline container", () => {
  ...
  it("publishes with the selected storage, shows status and copies the pages link", async () => {
    enableAuthenticatedPublishing(mocks);
    ...
    await confirmGitHubPagesPublish();

    await copyPublishedPagesLink();
  });
