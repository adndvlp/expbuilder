...
vi.mock("../../pages/ExperimentBuilder/providers/CanvasStylesProvider", () => ({
  default: ({
    children,
    experimentID,
  }: {
    ...
    <div data-testid="canvas-styles-provider" data-experiment-id={experimentID}>
      {children}
    </div>
  ),
}));
  ...
  default: (props: any) => (
    <div data-testid="timeline">
      ...
      <button
        type="button"
        onClick={() =>
          props.handleFileUpload({ target: { files: [] } } as any)
        }
      >
...
vi.mock("../../pages/ExperimentBuilder/components/ConfigurationPanel", () => ({
  default: () => <div data-testid="configuration-panel">Configuration</div>,
}));

vi.mock("../../pages/ExperimentBuilder/components/ExperimentPreview", () => ({
...
describe("ExperimentBuilder shell", () => {
  ...
  it("composes providers, renders normal builder panes, and wires shared file upload props", async () => {
    ...
    expect(screen.getByTestId("timeline")).toHaveTextContent("Timeline files 1");
