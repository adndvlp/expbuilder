...
describe("resume/jump startup protocol", () => {
  it("preserves a versioned jump request in the same session", () => {
    ...
    expect(localStorage.getItem("jsPsych_currentSessionId")).toBe(
      "old-session",
    );
    expect(localStorage.getItem("jsPsych_participantNumber")).toBe("7");
