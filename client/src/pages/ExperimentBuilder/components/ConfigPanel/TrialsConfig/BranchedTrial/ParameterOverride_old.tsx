/**
 * ParameterOverride Component
 *
 * Handles parameter overriding for branch conditions.
 * This component is responsible for:
 * - Displaying parameter override rows
 * - Managing parameter values (typed, CSV, or none)
 * - Supporting both normal plugins and DynamicPlugin
 */
...
function ParameterOverride({
  ...
  if (!condition.nextTrialId || isJumpCondition) {
    return null;
  }
  ...
  return (
    <>
      ...
      {customParamKeys.map((paramKey) => {
        ...
        return (
          <tr key={`param-${paramKey}`}>
            ...
            <td
              colSpan={selectedTrial?.plugin === "plugin-dynamic" ? 5 : 3}
            ></td>
            {/* Columna X vacía */}
            <td></td>
      ...
      {(isTargetDynamic || canAddMoreParams) && (
        <tr>
          {/* Columnas vacías para las columnas de reglas + botón X */}
          <td colSpan={selectedTrial?.plugin === "plugin-dynamic" ? 5 : 3}></td>
          {/* Columna X vacía */}
          <td></td>
          {/* Columna THEN Go To vacía */}
          <td></td>
