...
describe("compileLocalExperiment", () => {
  it("builds the runnable local artifact without React state", async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ tokens: [], separator: "_" })))
      .mockResolvedValueOnce(new Response(JSON.stringify({ timeline: [] })));
    ...
    expect(fetchImpl.mock.calls.map(([url]) => url)).toEqual([
      ...
      "http://127.0.0.1:4321/api/trials-metadata/E1",
    ]);
  });
});
