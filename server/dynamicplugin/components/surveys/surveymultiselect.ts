...
class SurveyMultiSelectPlugin {
        ...
        for (var i2 = 0; i2 < trial.questions.length; i2++) {
          if (trial.questions[i2].required) {
            if (
              ...
              display_element
                .querySelector("#jspsych-survey-multi-select-" + i2 + " input")
  ...
  create_simulation_data(trial, simulation_options) {
    ...
    for (const q of trial.questions) {
      ...
      const selections = this.jsPsych.randomization.sampleWithoutReplacement(
        q.options,
        n_answers
      );
    ...
    for (let i = 0; i < answers.length; i++) {
      for (const a of answers[i][1]) {
        this.jsPsych.pluginAPI.clickTarget(
          display_element.querySelector(
            `#jspsych-survey-multi-select-response-${i}-${trial.questions[
              i
            ].options.indexOf(a)}`
          ),
