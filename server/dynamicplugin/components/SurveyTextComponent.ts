...
/**
 * SurveyTextComponent - handles text input surveys
 * Stores response data internally, exposes via getters
 */
class SurveyTextComponent {
  private jsPsych: any;
  ...
  render(display_element: HTMLElement, trial: any) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      var question_index = this.question_order[i];
      html +=
    ...
    surveyContainer
      ...
      .addEventListener("submit", (e) => {
        ...
        for (var index = 0; index < trial.questions.length; index++) {
          ...
          var q_element = (
            surveyContainer.querySelector(
              "#jspsych-survey-text-" + index
            ) as HTMLElement
          ).querySelector("textarea, input") as
            | HTMLInputElement
            | HTMLTextAreaElement;
          var val = q_element.value;
          var name: any = q_element.attributes["data-name" as any].value;
          if (name == "") {
            name = id;
          }
          var obje: any = {};
...
export { SurveyTextComponent as default };
